...
def parse_departures(predictions_response: dict) -> pd.DataFrame:
    """
    Departures from selected stop: Destination, Scheduled/Actual Estimated Departure Time, Status.
    """
...
def parse_near_term_arrivals(
    ...
    """Arrivals to selected stop in next 10 minutes with Current Stop."""
...
def parse_future_arrivals(
    ...
    """Arrivals to selected stop in next 60 minutes with Current Stop."""
    raw = _parse_arrivals_raw(predictions_response)
    if raw.empty:
        return _empty_arrivals_df()
    vehicle_to_stop = _vehicle_to_stop_map(vehicles_response)
...
def _next_stop_by_vehicle(predictions_response: dict) -> dict:
    """From predictions at all stops, for each vehicle_id return next stop info."""
    if predictions_response.get("error"):
        return {}
    payload = predictions_response
    included = _build_included_lookup(payload.get("included", []))
    ...
    for item in payload.get("data", []):
        ...
        entry = {"stop_name": stop_name, "expected_time": pred_dt, "minutes_behind": minutes_behind}
...
def parse_vehicles_for_map_enriched(
    ...
    """Map vehicles with destination, direction, next_stop_name, etc. for hover."""
    ...
    for item in vehicles_response.get("data", []):
        ...
        if bearing is not None:
            try:
                ...
                bearing = None
        out.append({"lat": lat_f, "lon": lon_f, "bearing": bearing, "vehicle_id": item.get("id", "")})
    return out


def _decode_shape_to_lons_lats(item: dict) -> tuple[list[float], list[float]] | None:
    """Decode one shape resource to (lons, lats)."""
    attrs = item.get("attributes") or {}
...
def parse_red_line_shape(shapes_response: dict) -> list[tuple[list[float], list[float]]]:
    """Extract route geometry for map. Returns list of (lons, lats) per shape."""
...
def parse_route_shapes_merged(shapes_response: dict) -> tuple[list[float], list[float]] | None:
    """Return single merged geometry (lons, lats) using longest shape."""
...
def parse_merged_shapes_by_route(responses_by_route: dict[str, dict]) -> dict[str, tuple[list[float], list[float]]]:
    """Given route_id -> shapes response, return route_id -> single (lons, lats)."""
    out = {}
