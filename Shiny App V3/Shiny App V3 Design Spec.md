# Requirements for Shiny App V3 of MBTA Red Line
...
- Contain all original functionality of Shiny App V1, with the exception of the auto refresh feature
...
- All functionality of the Shiny App V1, except the auto refresh feature
...
- Display the Commuter Report in Markdown on a split screen in the app window, with the commuter report on the right side and the App V1 tables and live map slid over on the left side
- Controls to resize the split screen
- A button to generate a .docx of the Commuter Report in a subfolder for vaulting, with naming convention "MBTA Red Line Commuter Report Year.Month.Day-HHMM" with times logged in Eastern Standard Time
- Progress indicators that pop up when each button is pressed and disappear when the requested action is complete
